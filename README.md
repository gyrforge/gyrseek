...
| **Shai-Hulud: Bioinformatics & MCP wave** | PyPI | T2: `__init__.py` import hook fires on package import | ❌ | Deferred to import — outside the sandbox window. |
...
| **Shai-Hulud: Red Hat Cloud Services npm** | npm | T5: `preinstall` hook runs `node index.js` during `npm install` | ✅ | **Network:** new outbound connection to `github.com/oven-sh/bun/releases` (Bun download). **Bun execve:** `bun\|run\|_index.js`. Both flagged vs. baseline. |
...
| **Shai-Hulud: @antv ecosystem worm** (639 versions, 323 packages) | npm | T5: `preinstall` hook on each worm-republished package | ✅ | **Network:** new outbound connection to `github.com/oven-sh/bun/releases`. **Bun execve:** `bun\|run\|_index.js`. Each worm-injected package triggers the same signals inside the sandbox. |
...
| **Shai-Hulud: Intercom npm compromise** | npm | T6: `preinstall` runs `node setup.mjs`, downloads `router_runtime.js` | ✅ | **Network:** new outbound connection to `github.com/oven-sh/bun/releases` for `bun-linux-x64.zip`. **Bun execve:** `bun\|run\|router_runtime.js`. |
...
| **Shai-Hulud: Intercom PHP / Packagist** | PHP | T7: Composer plugin `post-install-cmd` runs shell script, downloads Bun | ❌ | PHP/Packagist is not a supported manager. Outside gyrseek's scope. Network IoC if it were in scope: `zero.masscan.cloud:443`. |
...
Several PyPI waves (including Shai-Hulud: Hades/Miasma, Shai-Hulud: Bioinformatics/MCP, TeamPCP LiteLLM, and TeamPCP Telnyx) use `.pth` files and import hooks specifically to survive install-time scanners. Telnyx (T26) avoided postinstall hooks entirely, placing `FetchAudio()` and `setup()` calls at module scope in `_client.py` — the payload only fires on `import telnyx`, not during `pip install`. The sequence:
