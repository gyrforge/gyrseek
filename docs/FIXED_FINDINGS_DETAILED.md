...
### Finding 292: README "silently clamped" inaccuracy for min_baseline_age_hours
- **Root Cause**: README.md config table row said "Values below 24h are silently clamped to the 24h security floor." The code at `src/lib.rs:258-262` emits an explicit `println!` warning when clamping occurs — not silent. This could cause operators to miss the warning thinking it was expected silent behaviour.
- **Fix**: Updated the README table row to say "clamped (with a warning)" to accurately reflect the emitted warning.

### Finding 298: AGENTS.md incorrectly described floor enforcement location in fetch_history_with_baselines
- **Root Cause**: AGENTS.md stated "the floor is enforced in all three code paths of `fetch_history_with_baselines`". FIXED #247 had already centralized the 24h minimum age floor to config-parse time at `src/lib.rs:257-262`; `fetch_history_with_baselines` receives a pre-clamped value and has no inline `HARD_MINIMUM_AGE_HOURS` check. The stale documentation pointed maintainers to the wrong location.
- **Fix**: Updated AGENTS.md to correctly state the floor is enforced at config-parse time in `src/lib.rs`, and the clamped value is passed through to `fetch_history_with_baselines`.

### Finding 302: Missing `.trim()` on per-package allowlist map keys in `load_policy_config`
- **Root Cause**: The `AllowlistEntry::PerPackage` branches for `ip_allowlist` (line ~234) and `domain_allowlist` (line ~269) in `src/lib.rs` called `entry(pkg.clone())` verbatim, without trimming the package key. Every other map-key consumer in `load_policy_config` applies `.trim()`: `baseline_overrides` at line ~285, `min_baseline_age_hours` at line ~317, `new_package_exemptions` at line ~334. A YAML key with leading or trailing whitespace (e.g. `" requests "`) would create a map entry under `" requests"` which never matches `plan.package` (`"requests"`) at filter call time — the allowlisted IPs/domains silently become unreachable with no warning. The bug was introduced in the same session that added per-package allowlist support.
- **Fix**: Added `let pkg = pkg.trim().to_string(); if pkg.is_empty() { continue; }` to both `PerPackage` branches, consistent with all other map-key consumers in the function.

### Finding 303: `deserialize_mixed_allowlist` was a no-op wrapper
- **Root Cause**: `fn deserialize_mixed_allowlist` at `src/lib.rs:65-70` had exactly one line of body: `Vec::<AllowlistEntry>::deserialize(d)`. This is identical to what serde generates for `#[serde(default)]` on a `Vec<AllowlistEntry>` field. The `#[serde(default, deserialize_with = "deserialize_mixed_allowlist")]` annotations on `ip_allowlist` and `domain_allowlist` added noise and a false impression of custom deserialization logic while doing nothing. `AllowlistEntry` already has `#[serde(untagged)]` which handles the mixed global/per-package YAML sequence natively.
- **Fix**: Removed `fn deserialize_mixed_allowlist` and both `deserialize_with = "deserialize_mixed_allowlist"` annotations; replaced with plain `#[serde(default)]`. Net change: -6 lines.

### Finding 304: Per-package allowlist key `"*"` silently collided with the global namespace
- **Root Cause**: In `load_policy_config`, `AllowlistEntry::PerPackage` processing for both `ip_allowlist` and `domain_allowlist` only checked `pkg.is_empty()` before calling `entry(pkg.clone())`. The reserved key `"*"` is used by the Global branch to store entries that apply to all packages. A YAML entry `- "*": ["1.2.3.4"]` in per-package position would insert into `ip_allowlist["*"]`, silently merging with global entries. Operators writing `"*"` intending global scope had their entries applied globally (accidentally correct); operators writing it by mistake got a confusing merge with no diagnostic. The same gap existed in `parse_list_map` for `git_clone_allowlist`, `artifact_allowlist`, `sensitive_file_access_allowlist`, and `process_exec_allowlist` — these four do not use a `"*"` global bucket, so a `"*"` key would create a dead entry never reached at lookup time.
- **Fix**: Added `if pkg == "*" { println!(...); continue; }` guard after the `is_empty()` check in both `ip_allowlist`/`domain_allowlist` PerPackage branches and in `parse_list_map`.

### Finding 312: Dead `get("*")` global-fallback branches in four parse_list_map-based filter functions
- **Root Cause**: When adding the `"*"` global-fallback to all six allowlist filter functions earlier in this session, the same `.get("*").chain(get(package_name))` pattern from `filter_allowlisted_new_connections` was applied to `filter_allowlisted_sensitive_reads`, `filter_allowlisted_process_exec_signatures`, `filter_allowlisted_artifact_findings`, and `filter_allowlisted_git_clone_signatures`. However, those four allowlists are populated exclusively via `parse_list_map`, which (per Fix 304) explicitly rejects `"*"` as a reserved key with a warning and `continue`. The `get("*")` call always returns `None` in production — dead code. The four "global applies to all packages" tests added this session bypassed `parse_list_map` by constructing maps directly with `"*"` keys, creating misleading coverage that suggested global support was intentional and working. A future contributor could unknowingly rely on this apparent global support.
- **Fix**: Reverted the four filter functions to `get(package_name)` only, matching the actual contract that these allowlists are per-package only. Removed the four dead tests (`global_sensitive_reads_allowlist_applies_to_all_packages`, `global_process_exec_allowlist_applies_to_all_packages`, `global_artifact_allowlist_applies_to_all_packages`, `global_git_clone_allowlist_applies_to_all_packages`). The `ip_allowlist`/`domain_allowlist` filter functions (`filter_allowlisted_new_connections`, `filter_domain_allowlisted_new_connections_with`) correctly retain their `get("*")` chains because those allowlists use a `Vec<AllowlistEntry>` with a `Global` branch that populates `"*"` at config-load time.

### Finding 310: Whitespace-variant per-package allowlist keys silently merge or overwrite after trim
- **Root Cause**: Fix 302 added `.trim()` to per-package allowlist keys in `load_policy_config`. A side effect is that two YAML map keys differing only by surrounding whitespace — e.g. `"requests"` and `" requests"` — both normalize to `"requests"`. For ip/domain allowlist PerPackage branches, `.entry(pkg).or_default().insert(...)` merges their entries silently. For `parse_list_map` (git_clone_allowlist, artifact_allowlist, sensitive_file_access_allowlist, process_exec_allowlist), the second key's `result.insert(package, list)` overwrites the first. In both cases no warning was emitted. This is the same class as OPEN #277 for `new_package_exemptions`. While the config is a trusted boundary, silent merges/overwrites are an operator footgun with no diagnostic.
- **Fix**: Added `if pkg != pkg_raw && map.contains_key(&pkg)` collision check with a `println!` warning before the merge/insert at all three sites: ip_allowlist PerPackage, domain_allowlist PerPackage, and `parse_list_map`. The rename `pkg` → `pkg_raw` for the raw YAML key makes the comparison self-documenting.

### Finding 305: IPv4-mapped IPs stored un-normalized in ip_allowlist; re-canonicalized at every lookup
- **Root Cause**: At config-load time, `addr.to_string()` was called on the parsed `IpAddr`. For an input of `"::ffff:1.2.3.4"`, Rust's `IpAddr::V6.to_string()` preserves the `::ffff:` prefix rather than collapsing it. `filter_allowlisted_new_connections` compensated by re-parsing every stored allowlist entry through `normalize_ip_string` via `filter_map` at each call, adding a parse+normalize pass over the entire allowlist on every invocation. This was a latent inefficiency introduced when per-package allowlist support landed: the equivalent normalization was already present for connection IPs at extract time, but not at config-store time.
- **Fix**: At config-load time call `scanning::normalize_ip_string(&s)` instead of `addr.to_string()` for both Global and PerPackage ip branches. In `filter_allowlisted_new_connections` replace the `filter_map` re-canonicalization with `.map(|s| s.as_str()).collect()` since entries are now guaranteed canonical. Updated `ip_allowlist_matches_across_ipv4_mapped_and_bare_forms` test: the second case previously stored `"::ffff:203.0.113.5"` directly in the map (relying on the filter to normalize it); updated to store the already-collapsed `"203.0.113.5"` reflecting the new contract that allowlist maps hold normalized values.
- **Residual**: The config-load path still double-parses each IP: `s.parse::<IpAddr>()` validates the string (discarding the parsed result), then `normalize_ip_string(&s)` re-parses from scratch internally. Threading the parsed `IpAddr` into normalization would eliminate the second parse but would require changing `normalize_ip_string`'s signature, rippling to all callers. This residual is tracked as WONT_FIX #342.

### Finding 314: `parse_list_map` insert semantics diverged from ip/domain PerPackage merge semantics
- **Root Cause**: Fix 310 added collision warnings to all three allowlist key sites (ip_allowlist PerPackage, domain_allowlist PerPackage, `parse_list_map`). The ip/domain branches already used `entry().or_default().insert()` (merge semantics). `parse_list_map` still used `result.insert(package, list)` (last-write-wins). After trimming, two whitespace-variant keys could collide: for ip/domain the entries merged silently; for `parse_list_map`-based allowlists the second key's value replaced the first. The warning message said "overwriting" which was accurate for `parse_list_map` but misleading given the intent to have consistent behavior. Switching to `entry().or_default().extend(list)` also required an explicit type annotation (`let mut result: HashMap<String, HashSet<String>>`) since the compiler could no longer infer the value type without a concrete insert.
- **Fix**: Changed `result.insert(package, list)` to `result.entry(package).or_default().extend(list)` in `parse_list_map`. Added `let mut result: HashMap<String, HashSet<String>> = HashMap::new()` annotation. Updated the collision warning from "overwriting" to "merging" to consistently describe the behavior across all three sites.

### Finding 315: `domain_allowlist` accepted `"*"` as a value — silently matched nothing
- **Root Cause**: `domain_is_allowlisted` in `src/scanning.rs` checks each allowlist entry against an FCrDNS-resolved hostname using exact equality or `.ends_with(".<allowed>")`. The literal string `"*"` satisfies neither condition — no real domain equals `"*"` and no domain ends with `".*"`. However, both the Global branch (`if !normalized.is_empty() { global.push(normalized) }`) and the PerPackage branch stored the value without complaint. An operator writing `domain_allowlist: ["*"]` or `"pkg": ["*"]` would get a silently dead entry: the config loaded, no error was emitted, but no domain ever matched — a confusing no-op with no diagnostic.
- **Fix**: Added `if normalized == "*" { println!("⚠️ ..."); continue/skip }` guards in both the Global branch and the PerPackage per-domain loop in `load_policy_config`. The warning explains that `"*"` is not a valid domain matcher.

### Finding 316: `filter_allowlisted_sensitive_reads` argument order diverged from all peer filter functions
- **Root Cause**: Five peer filter functions (`filter_allowlisted_new_connections`, `filter_allowlisted_process_exec_signatures`, `filter_allowlisted_artifact_findings`, `filter_allowlisted_git_clone_signatures`, `filter_domain_allowlisted_new_connections_with`) all use the signature pattern `(package_name, items, allowlist_map)`. `filter_allowlisted_sensitive_reads` was introduced with `(reads, package_name, allowlist_map)` — `reads` and `package_name` swapped. The inconsistency was a footgun: a caller passing `(reads, package_name, allowlist)` by analogy with the other five would silently pass `reads` as `package_name`, causing all sensitive-file allowlist entries to be looked up under the wrong key and never match.
- **Fix**: Reordered signature to `(package_name: &str, reads: Vec<String>, allowlist_map: &HashMap<...>)`. Updated the production call site in `src/scanning.rs` and both test call sites.

### Finding 317: Finding `300a` used letter-suffix ID violating flat-numeric-namespace rule
- **Root Cause**: A finding was filed as `300a` to avoid collision with FIXED #300 (which didn't exist) or to distinguish a sub-finding variant. The project's flat-numeric-namespace rule (documented in AGENTS.md) requires plain integers with no prefixes or suffixes, and IDs must be globally unique across FIXED, WONT_FIX, and OPEN finding files. A letter suffix `a` is not a valid ID under this convention and could create ambiguity with a future Finding 300 (an integer that starts with 300).
- **Fix**: Renamed `300a` → `313` (the next available integer across all three finding files at the time of the fix) in `WONT_FIX_FINDINGS.md`. The `WONT_FIX_FINDINGS_DETAILED.md` file did not have a matching `300a` entry so no rename was needed there.

### Finding 318: OPEN #299 stale after WONT_FIX #252 renamed to #313
- **Root Cause**: OPEN #299 tracked a duplicate-ID collision: both FIXED_FINDINGS.md and WONT_FIX_FINDINGS.md contained a finding #252. Fix 317 in this session resolved the collision by renaming WONT_FIX #252 → #313. However OPEN #299 was never closed. The detailed entry in OPEN_FINDINGS_DETAILED.md still said "Renumber to next available ID (currently 300)" and referenced `300a`, both of which diverged from the actual outcome (renamed to 313).
- **Fix**: Removed OPEN #299 from OPEN_FINDINGS.md and OPEN_FINDINGS_DETAILED.md. Added FIXED #318 documenting the resolution: the collision was eliminated by Fix 317.

### Finding 319: `domain_allowlist` README row ambiguous when read in isolation (resolved WONT_FIX #307)
- **Root Cause**: The `domain_allowlist` README table row read "Same **global / per-package** mixed syntax as `ip_allowlist`." The protection guarantee ("effective set = global ∪ per-package; per-package entries never reduce global protection") was only stated in the `ip_allowlist` row. A reader scanning the `domain_allowlist` row without reading `ip_allowlist` would not see the guarantee. Filed as WONT_FIX #307 on the basis that the guarantee is "inherited by reference." The AppSec recommendation was to make it explicit with a one-line change — trivially cheap and removes genuine ambiguity.
- **Fix**: Updated `domain_allowlist` README row to "Same semantics and syntax: effective set = global ∪ per-package (see `ip_allowlist`)." Removed WONT_FIX #307 from WONT_FIX_FINDINGS.md and WONT_FIX_FINDINGS_DETAILED.md.

### Finding 322: No test for `"*"` as a domain value in global and per-package positions
- **Root Cause**: Fix 315 added `if normalized == "*"` guards in both the Global branch (rejects `"*"` from the global domain list) and the PerPackage per-domain loop (rejects `"*"` from per-package domain lists). The existing test `per_package_allowlist_star_key_rejected_for_ip_domain_and_list_map` placed `"*"` in the KEY position of `domain_allowlist` (i.e., `- "*": [...]`), which exercises the key-rejection guard added by Fix 304, not the value-rejection guard added by Fix 315. A regression removing either of the Fix 315 value guards would pass CI unnoticed: the guard silently drops the value without altering map structure, so an absent guard would just add `"*"` to the allowlist set — a live dead entry with no test to catch it.
- **Fix**: Added `domain_allowlist_star_value_rejected_in_global_and_per_package_positions` test in `src/lib.rs` config_tests module. The test constructs YAML with `"*"` as a global domain value and `"*"` as a per-package domain value alongside a legitimate entry (`cdn.example.com`), then asserts: the global `"*"` bucket does not contain `"*"`, the per-package `"requests"` set does not contain `"*"`, and `"cdn.example.com"` is preserved.

### Finding 324: `domain_is_allowlisted` redundantly called `normalize_domain` per entry on every comparison
- **Root Cause**: `domain_is_allowlisted` called `normalize_domain(allowed)` inside the `for allowed in domain_allowlist` loop, allocating a new `String` per entry per IP lookup. `normalize_domain` runs `.trim().trim_end_matches('.').to_ascii_lowercase()` — three operations that produce a heap allocation each time. Allowlist entries are already normalized at config-load time (`src/lib.rs:280` Global branch, `src/lib.rs:314` PerPackage branch), so the re-normalization was pure waste. This is the same class of inefficiency that FIXED #305 addressed for IP normalization: `filter_allowlisted_new_connections` previously re-parsed every stored IP entry through `normalize_ip_string` at each call, which was moved to config-load time.
- **Fix**: Removed `let allowed = normalize_domain(allowed)` from the loop body. Replaced the `for` loop with `.iter().any(|allowed| normalized == *allowed || normalized.ends_with(&format!(".{}", allowed)))` — direct comparison against pre-normalized stored forms, no allocation per entry. Updated `domain_allowlist_normalization_matches_case_whitespace_and_trailing_dot` test to store the pre-normalized form (`"example.net"` instead of `" Example.NET. "`), reflecting the contract that allowlist maps hold canonical values.

### Finding 325: `sensitive_reads` per-package isolation test missing; `git_clone` and `process_exec` already existed
- **Root Cause**: The claim stated all four `parse_list_map`-based filter functions lacked per-package isolation tests. Investigation found `artifact_allowlist_does_not_leak_across_packages` (line 3204), `git_clone_allowlist_does_not_leak_across_packages` (line 3219), and `process_exec_allowlist_does_not_leak_across_packages` (line 3145) already existed. Only `filter_allowlisted_sensitive_reads` was genuinely missing a cross-package isolation test.
- **Fix**: Added `sensitive_reads_allowlist_does_not_leak_across_packages` test: constructs an allowlist with `aws-sdk → ["/home/user/.aws/credentials"]`, calls `filter_allowlisted_sensitive_reads("boto3", ...)`, and asserts the read is not allowlisted for the wrong package.
- **OPEN #77 resolution note**: OPEN #77 requested exactly this cross-package isolation test for `filter_allowlisted_sensitive_reads`. This fix closes it. Removed from OPEN_FINDINGS.md and OPEN_FINDINGS_DETAILED.md.

### Finding 326: AGENTS.md stale "summary tables synced across both main and detailed files" instruction
- **Root Cause**: AGENTS.md step 9 read "Remember to keep the summary tables synced across both the main and detailed files." The `_DETAILED.md` files (`FIXED_FINDINGS_DETAILED.md`, `WONT_FIX_FINDINGS_DETAILED.md`, `OPEN_FINDINGS_DETAILED.md`) contain only prose entries — no summary tables. Summary tables exist only in the main files. An agent following this instruction verbatim would look for a table to update in a detailed file, find none, and either error or waste cycles searching.
- **Fix**: Replaced the stale phrase with "Summary tables live only in the main files (`FIXED_FINDINGS.md`, `WONT_FIX_FINDINGS.md`, `OPEN_FINDINGS.md`); the `_DETAILED.md` files contain only prose entries." Removed OPEN #291 from OPEN_FINDINGS.md.

### Finding 328: Empty-string domain_allowlist values silently dropped without warning
- **Root Cause**: Both the Global branch (`lib.rs:285`) and the PerPackage per-domain loop (`lib.rs:320`) used `else if !normalized.is_empty() { ... insert }` — blank or whitespace-only values (which normalize to `""`) fell through both the `"*"` guard and the `is_empty()` check and were silently discarded. The IP allowlist path is inconsistent: `"".parse::<IpAddr>()` fails into the `Err` branch which always emits `"⚠️ [gyrseek] Ignoring invalid ip_allowlist entry (not an IP)"`. An operator writing `domain_allowlist: [""]` would see nothing, while `ip_allowlist: [""]` emits a warning.
- **Fix**: Replaced `else if !normalized.is_empty()` with an explicit `else if normalized.is_empty() { println!(...) } else { insert }` pattern in both the Global and PerPackage domain value paths. Global emits `"Ignoring blank domain_allowlist entry"`; PerPackage emits `"Ignoring blank domain_allowlist entry for package '<pkg>'"`, consistent with the package-context pattern used by the `"*"` value guard (Fix 315).

### Finding 329: Duplicated per-package key-validation skeleton in `load_policy_config`
- **Root Cause**: The `AllowlistEntry::PerPackage` branches for `ip_allowlist` and `domain_allowlist` in `src/lib.rs` each contained ~37–39 lines of identical key-validation logic: trim the raw key, skip empty keys with a warning, reject `"*"` with a warning, detect whitespace-variant collisions with a warning, then insert. The same skeleton also appeared in `parse_list_map`. Three independent copies of the same logic meant that a fix or improvement to the validation rules (e.g., a new reserved key category) had to be applied in three places simultaneously, with no compile-time guarantee of consistency.
- **Fix**: Extracted `fn validate_allowlist_pkg_key(pkg_raw: &str, allowlist_name: &str, existing: &HashMap<String, _>) -> Option<String>` helper. The function trims, rejects empty, rejects `"*"` with guidance text, warns on collision (where the existing map already has the trimmed key), and returns `Some(trimmed_key)` or `None`. All three sites (ip_allowlist PerPackage, domain_allowlist PerPackage, `parse_list_map`) were updated to call this helper, removing the duplicated blocks.

### Finding 330: Three identical `find_new_*` functions with no shared implementation
- **Root Cause**: `find_new_sensitive_reads`, `find_new_git_clone_signatures`, and `find_new_process_exec_signatures` in `src/scanning.rs` all had identical bodies: collect the current set, collect the baseline set, compute the set difference, sort, and return. The only difference between them was the element type — all three operated on `Vec<String>`. Three copies meant a latent inconsistency risk: a sort stability change or a tweak to the difference semantics applied to one function would silently not apply to the others.
- **Fix**: Extracted `fn find_new_items(current: &[String], baseline: &[String]) -> Vec<String>` — a single implementation using `HashSet` difference + sort. All three named functions now delegate to it with a one-line body.

### Finding 331: `release_burst_threshold` and `minimum_release_age_package` parsers were identical `Some(0)→None` blocks
- **Root Cause**: Both `release_burst_threshold` and `minimum_release_age_package` in `src/lib.rs` used the same three-arm pattern: `Some(0) → None + warning`, `Some(v) → Some(v)`, `None → None`. The code was copy-pasted verbatim with only the warning message changed. `release_burst_window_hours` was intentionally excluded: it returns `usize` (not `Option<usize>`) with a default of 24, so its zero-handling differs.
- **Fix**: Extracted `fn option_zero_to_none(val: Option<usize>, warn_msg: &str) -> Option<usize>` helper. Both callers replaced their match blocks with a single `option_zero_to_none(raw, "...")` call. `release_burst_window_hours` intentionally uses its own inline logic and is not affected.

### Finding 332: `"*"` domain value rejection warnings lacked actionable guidance
- **Root Cause**: Fix 315 added `"*"` value rejection in both the Global and PerPackage domain_allowlist branches, but the warning messages only said "does not match any domain" without telling the operator what to write instead. An operator who accidentally typed `"*"` expecting wildcard semantics would see the warning, remove the entry, and be left uncertain how to express "allow a domain". This is the same quality gap that prompted the fix to ip_allowlist key warnings to include "e.g. '1.2.3.4'".
- **Fix**: Updated both Global and PerPackage `"*"` domain value warning messages to append: `"Use exact domain names (e.g. 'pypi.org') or remove this entry."` — consistent with the specificity of guidance elsewhere in the config-parse warnings.

### Finding 333: No test for empty-string domain value in global or per-package position
- **Root Cause**: Fix 328 added explicit `else if normalized.is_empty()` branches with warnings for blank domain values. No test verified either the warning emission or that the entry was correctly excluded from the allowlist map. A regression silently removing the guard would pass CI: the behavior change (allowing a blank string into the map) would not fail any existing test because `domain_is_allowlisted` never matches a blank allowed entry against a real domain anyway — the test gap was in the warning path and the "was it excluded?" assertion.
- **Fix**: Added `domain_allowlist_empty_value_dropped_in_global_and_per_package_positions` test in `src/lib.rs` config_tests module. The test constructs YAML with `""` as a global domain value and `""` as a per-package domain value alongside a legitimate entry, then asserts: the global `"*"` bucket does not contain `""`, the per-package set does not contain `""`, and the legitimate entry is preserved.

### Finding 338: `parse_list_map` passed generic `"allowlist"` name in warnings
- **Root Cause**: `validate_allowlist_pkg_key` was called with the literal string `"allowlist"` for all four `parse_list_map` callers (`git_clone_allowlist`, `artifact_allowlist`, `sensitive_file_access_allowlist`, `process_exec_allowlist`). The ip_allowlist and domain_allowlist PerPackage branches passed their specific names (`"ip_allowlist"`, `"domain_allowlist"`). When an operator had a bad key in `git_clone_allowlist`, they saw `⚠️ [gyrseek] allowlist key "pkg" trimmed...` instead of the more informative `git_clone_allowlist key "pkg" trimmed...`.
- **Fix**: Added `name: &str` parameter to `parse_list_map`. All four call sites now pass their specific allowlist name. `validate_allowlist_pkg_key` receives the correct name for diagnostic output.

### Finding 339: Dead `global_ok` variable in test
- **Root Cause**: The `domain_allowlist_empty_value_dropped_in_global_and_per_package_positions` test (added for Fix #333) had a leftover `global_ok` binding that computed `cfg.domain_allowlist.get("*").is_none_or(|s| !s.contains(""))`. The subsequent `assert!` on the next line checked a semantically equivalent predicate (`!s.contains(&"".to_string())`). `global_ok` was never used; `let _ = global_ok` suppressed the unused-variable warning. This was an artifact of test refactoring where the assert was strengthened after the binding was written.
- **Fix**: Removed the `global_ok` binding entirely. Simplified to `!s.contains("")` (no `"".to_string()` allocation needed — `HashSet<String>::contains` accepts `&str` via the `Borrow` blanket impl).

### Finding 340: No whitespace-collision merge test for `parse_list_map`-based allowlists (also resolves OPEN #79)
- **Root Cause**: `per_package_allowlist_whitespace_collision_merges_both_ip_sets` (added for Fix #310) covered the ip_allowlist PerPackage branch only. The four `parse_list_map`-based allowlists share the same `validate_allowlist_pkg_key` → `entry().or_default().extend()` merge path, but no test verified that whitespace-variant keys (e.g. `"requests"` and `"  requests  "`) produce a merged set rather than a silently dropped second entry.
- **Fix**: Added `parse_list_map_whitespace_collision_merges_both_value_sets` test using `git_clone_allowlist` as the representative `parse_list_map` consumer. Verifies that both `https://github.com/psf/requests.git` and `https://github.com/psf/requests-cache.git` appear in the merged `"requests"` set.
- **OPEN #79 resolution note**: OPEN #79 also described three other attack-surface concerns for `parse_list_map`: (1) colon-injection in values, (2) comma-injection in package names, and (3) empty-value-collapsing. None of these apply in practice. `parse_list_map` accepts `HashMap<String, Vec<String>>` — the YAML deserializer (serde_yaml) has already parsed and validated the structure; colon and comma characters in values are handled natively by YAML list syntax and are never treated as delimiters by `parse_list_map`. For (3), empty values: `parse_list` at `src/lib.rs:158` calls `.filter(|s| !s.is_empty())` on items before returning, so empty strings are dropped before they reach the allowlist sets. These were not fixed because they were not present as bugs.

### Finding 341: Overly-permissive sensitive file allowlist value check fired once per read
- **Root Cause**: The `"*"` / `"/"` / `"*/"` guard in `filter_allowlisted_sensitive_reads` was inside an `.any()` closure that iterated over allowlist entries for each read path. With N reads being evaluated and one bad entry present, the `println!` warning fired N times — identical messages for each read path being tested against the bad entry. The values `"*"`, `"/"`, and `"*/"` are not blocked at `parse_list_map` load time (which validates keys, not values), so they survive into the filter.
- **Fix**: Added a post-load validation pass in `load_policy_config` immediately after the `sensitive_file_access_allowlist = parse_list_map(...)` call. Iterates over `iter_mut()`, calls `entries.retain(|v| ...)` to drop bad values, and emits one warning per bad entry with the package name in context. Removed the runtime check from `filter_allowlisted_sensitive_reads` entirely — the filter now only applies allowlist entries that passed validation at load time.

### Finding 351: ROADMAP.md completed section omitted per-package allowlist feature
- **Root Cause**: Post-change policy step 7 requires ROADMAP.md updates for architectural changes. The mixed global/per-package allowlist syntax across all six allowlist types — added across multiple sessions — was never recorded in the completed section. A developer reading ROADMAP.md had no way to know this capability existed.
- **Fix**: Added a completed entry documenting the mixed global/per-package syntax, the `effective = global ∪ per-package` semantics for ip/domain, per-package-only constraints for the other four, and config-load-time key/value validation.

### Finding 352: FIXED #340 closure of OPEN #79 lacked rationale for original injection concerns
- **Root Cause**: OPEN #79 described three attack-surface concerns for `parse_list_map`: colon-injection in values, comma-injection in package names, and empty-value-collapsing. FIXED #340 resolved the whitespace-collision gap (a different concern) and noted it "also resolves OPEN #79" without explaining why the three original concerns were not applicable. A future reviewer reading the closure could not determine whether they were fixed, inapplicable, or overlooked.
- **Fix**: Added an "OPEN #79 resolution note" to Finding 340's detailed entry explaining: (1) `parse_list_map` accepts `HashMap<String, Vec<String>>` — serde_yaml has already parsed and validated structure, so colon/comma are never treated as delimiters; (2) `parse_list` at `src/lib.rs:168` calls `.filter(|e| !e.is_empty())`, so empty strings are dropped before reaching allowlist sets.

### Finding 370: `validate_allowlist_pkg_key` `"*"` rejection message lacked additive-semantics guidance
- **Root Cause**: The message `"use a bare global entry instead"` correctly redirected the operator but didn't explain *why*: per-package entries are additive to global entries, so global entries already apply to all packages and there's no need to also add `"*"` as a per-package key. An operator unfamiliar with this model might add `"*"` to every per-package block assuming it was needed for cross-package scoping.
- **Fix**: Appended `"Per-package entries are additive to global entries — global entries already apply to all packages."` to the `has_global_variant=true` message path.

### Finding 371: Startup message for `git_clone_allowlist` counted keys not targets
- **Root Cause**: The startup message said `"Loaded N allowlisted git clone target(s)"` but used `policy.git_clone_allowlist.len()` — a count of *package keys*, not the total number of clone target URLs. If `my-pkg` had 3 allowed clone targets, the message said "1 allowlisted git clone target(s)" — misleading for operators auditing their config at startup. The other three `parse_list_map`-based allowlists (artifact, sensitive_file_access, process_exec) already used "for N package(s)" wording, making this one inconsistent.
- **Fix**: Changed wording to `"Loaded git clone allowlist for N package(s)"` — consistent with the other three allowlists and accurate about what N represents.

### Finding 367: Bare TLD domain_allowlist entries created overly-broad suffix match
- **Root Cause**: `domain_is_allowlisted` at `scanning.rs:299` checks `normalized.ends_with(&format!(".{}", allowed))`. A bare TLD entry like `"com"` (no dot) expands to `.ends_with(".com")`, matching `evil.com`, `attacker.com`, and any other `.com` domain — completely voiding the FCrDNS trust model for affected packages. No validation at config-load time rejected such entries. WONT_FIX #337 covers the intentional subdomain matching (`"amazonaws.com"` → `*.amazonaws.com`); a bare TLD is distinct and almost certainly a typo or misconfiguration.
- **Fix**: Added `!normalized.contains('.')` check in both the `AllowlistEntry::Global` branch and the `AllowlistEntry::PerPackage` inner loop at config-load time. A dot-free normalized entry is dropped with a warning: `"a bare label without a dot would match all subdomains of any TLD. Use a fully-qualified domain"`. Added `domain_allowlist_bare_tld_rejected_in_global_and_per_package_positions` test.

### Finding 368: `sensitive_file_access_allowlist` value-retain left empty packages in map
- **Root Cause**: The `entries.retain(|v| ...)` loop at lib.rs:444–457 dropped overly-permissive values (`"*"`, `"/"`, `"*/"`, `"/*"`) from each package's `HashSet`. If all values for a package were rejected, the `HashSet` became empty but the package key remained in the map. The startup summary message at lib.rs:1682 calls `.len()` on the map and counted that key as a "loaded" package — misleading the operator into believing protection was active. `parse_list_map` (FIXED #363) handles the symmetric case (blank values at parse time); this is the same gap one step later in the pipeline.
- **Fix**: Added a `sensitive_file_access_allowlist.retain(|pkg, entries| ...)` pass immediately after the value-filter loop. Packages whose `HashSet` is empty after filtering are removed from the map with a per-package warning: `"has no valid entries after filtering; no allowlist protection will be applied"`. Added `sensitive_file_access_allowlist_all_values_filtered_drops_key` test.

### Finding 376: `option_zero_to_none` lacked a dedicated unit test
- **Root Cause**: `option_zero_to_none` originally had two call sites; coverage via callers was judged sufficient and tracked as WONT_FIX #348. This PR added a third call site (`release_burst_window_hours`), raising the stakes: a regression in the `Some(0) → None` arm or the pass-through arm would silently affect all three config fields (`release_burst_threshold`, `release_burst_window_hours`, `minimum_release_age_package`) with no test directly exercising the function's three branches. The indirect coverage argument holds for a one-liner; `option_zero_to_none` has three distinct arms and a side-effect (warning print) that caller tests do not individually target.
- **Fix**: Added `option_zero_to_none_direct` test in `src/lib.rs` config_tests module: asserts `Some(0) → None`, `Some(42) → Some(42)`, `None → None`. WONT_FIX #348 narrowed to `parse_list` only.

### Finding 374: Config-loading helper layer undocumented in ARCHITECTURE.md
- **Root Cause**: `docs/ARCHITECTURE.md` described allowlist decision semantics (step 5 of the Decision Model and scattered bullet points) but never described the helper subsystem that enforces config-load-time invariants: `validate_allowlist_pkg_key` (key hygiene, `"*"` rejection with context-sensitive guidance, whitespace-variant collision detection), `parse_list_map` (per-package map loading with merge semantics and empty-set warnings), and `option_zero_to_none` (zero-to-disabled bridging). A reader of the architecture doc could understand what allowlists do at runtime but not how the config is validated and normalised on startup, or why these three helpers were extracted.
- **Fix**: Added a "Config-loading helpers" subsection under Core Components describing each helper's responsibility, the validation invariants it enforces, and the call sites that use it.

### Finding 362: `find_new_items` documented with wrong signature in AGENTS.md
- **Root Cause**: AGENTS.md line 140 described `find_new_items(current: &[String], baseline: &[String]) -> Vec<String>` — a slice-based signature from an earlier design draft. The actual implementation at `src/scanning.rs:1567` takes `&HashSet<String>` (required for O(n) set-difference via `.difference()`). Any code-generating agent following the AGENTS.md signature would produce either uncompilable code or unnecessary `Vec → HashSet` conversions.
- **Fix**: Updated AGENTS.md to the correct signature: `find_new_items(current: &HashSet<String>, baseline: &HashSet<String>) -> Vec<String>`.

### Finding 363: `parse_list_map` silently drops per-package entries with empty value sets
- **Root Cause**: When all values for a package key are blank/whitespace (filtered by `parse_list`), the resulting `HashSet` is empty and the `if !list.is_empty()` guard skips insertion with no diagnostic. An operator who writes `git_clone_allowlist: { my-pkg: [] }` (or a list of blank strings) sees no warning and believes `my-pkg` has per-package protection. The same silent drop applied to all four `parse_list_map`-based allowlists.
- **Fix**: Inverted the guard: when `list.is_empty()`, emit `⚠️ [gyrseek] {name} package key {:?} has no valid entries; no allowlist protection will be applied for this package`. When non-empty, insert as before.

### Finding 364: `validate_allowlist_pkg_key` gave misleading `"*"` guidance for parse_list_map-based allowlists
- **Root Cause**: The `"*"` rejection message said `"use a bare global entry instead"` — correct for `ip_allowlist`/`domain_allowlist` (which have `AllowlistEntry::Global`), but actively misleading for the four `parse_list_map`-based allowlists (`git_clone_allowlist`, `artifact_allowlist`, `sensitive_file_access_allowlist`, `process_exec_allowlist`) which have no global variant. An operator following the advice would write invalid YAML and get no protection.
- **Fix**: Added `has_global_variant: bool` parameter to `validate_allowlist_pkg_key`. The two ip/domain PerPackage call sites pass `true` and get the original guidance. The `parse_list_map` call site passes `false` and emits: `"this allowlist does not support a global wildcard — use a specific package name"`.

### Finding 365: Missing startup summary messages for three allowlists
- **Root Cause**: `artifact_allowlist`, `sensitive_file_access_allowlist`, and `process_exec_allowlist` emitted no `ℹ️ [gyrseek] Loaded …` startup message. All other config sections (ip_allowlist, domain_allowlist, git_clone_allowlist, baseline_overrides, min_baseline_age_hours, new_package_exemptions, internal_package_exemptions) emit a count on load. An operator configuring these three allowlists had no confirmation they were parsed, and a YAML indentation error producing an empty map was undetectable until a CI pipeline unexpectedly blocked.
- **Fix**: Added three `ℹ️ [gyrseek] Loaded {allowlist} for N package(s) from {config_path}` messages in `run()`, inserted after the git_clone_allowlist message for logical grouping.

### Finding 358: `pkg.clone()` in per-package allowlist inner loops
- **Root Cause**: In the ip_allowlist and domain_allowlist PerPackage branches of `load_policy_config`, `pkg.clone()` was called inside the inner `for s in ips`/`for s in domains` loop — creating N string clones per package instead of one. `HashMap::entry()` needs ownership of the key only on first insertion; subsequent iterations touch the existing entry via `set`.
- **Fix**: Hoisted `let set = ip_allowlist.entry(pkg.clone()).or_default()` (and the domain equivalent) before the inner loop. The loop body now calls `set.insert(...)` directly, eliminating N−1 redundant clones per package.

### Finding 359: `release_burst_window_hours = 0` not normalised to the 24h default
- **Root Cause**: `release_burst_threshold` and `minimum_release_age_package` both used `option_zero_to_none` to map `Some(0)` → `None`. `release_burst_window_hours` returned `usize` (not `Option<usize>`) and was documented as "intentionally excluded." A config value of `0` therefore silently produced a zero-hour burst window — every burst check would fire on all packages regardless of timing — instead of falling back to the 24h default.
- **Fix**: Applied `option_zero_to_none(cfg.release_burst_window_hours, "Ignoring invalid release_burst_window_hours=0; using default 24").unwrap_or(24)`. The type difference is bridged cleanly by `unwrap_or`. Updated AGENTS.md to remove the stale "intentionally excluded" note.

### Finding 360: `"/*"` bypassed the sensitive_file_access_allowlist overly-permissive value guard
- **Root Cause**: The `retain` guard in `load_policy_config` checked for `"*"`, `"/"`, and `"*/"` but not `"/*"`. A config entry `"/*"` passed the guard, was stored in the allowlist, and then never matched any real path (no POSIX path begins with `/*`) — a silent dead entry with no operator warning.
- **Fix**: Added `"/*"` to the guard condition: `if t == "*" || t == "/" || t == "*/" || t == "/*"`. Consistent with the three already-rejected forms.

### Finding 361: No config-load test for IPv4-mapped normalization
- **Root Cause**: FIXED #305 documented that `normalize_ip_string` collapses `::ffff:x.x.x.x` → bare IPv4 at config-load time so filter lookups use direct `HashSet` membership. The existing test (`parses_per_package_ip_and_domain_allowlist`) constructed the map directly with pre-normalized values — it never called `load_policy_config`. The glue between `load_policy_config` and `normalize_ip_string` was untested; a regression could silently store raw `::ffff:` forms and cause bare-IPv4 connection IPs to miss the allowlist at scan time.
- **Fix**: Added `ip_allowlist_config_load_normalizes_ipv4_mapped_ipv6` test. Writes a YAML config with a global `"::ffff:203.0.113.5"` entry and a per-package `requests: ["::ffff:198.51.100.1"]` entry, calls `load_policy_config`, and asserts the stored forms are bare IPv4 and the raw `::ffff:` forms are absent.

### Finding 334: ARCHITECTURE.md allowlist section omitted per-package syntax and Global ∪ PerPackage semantics
- **Root Cause**: `docs/ARCHITECTURE.md` described the six allowlists without mentioning that `ip_allowlist` and `domain_allowlist` support a mixed global+per-package syntax, that the effective set is `global("*") ∪ per-package(pkg)`, or that the four `parse_list_map`-based allowlists (`git_clone_allowlist`, `artifact_allowlist`, `sensitive_file_access_allowlist`, `process_exec_allowlist`) are per-package only with no global variant. The Decision Model step 5 listed only `ip_allowlist` and `domain_allowlist` and omitted the other four. A maintainer reading ARCHITECTURE.md for the authoritative design description would not learn the effective-set semantics from that document alone.
- **Fix**: Updated the allowlist section to document the mixed Global/PerPackage variant for ip/domain, the `effective = global ∪ per-package` guarantee, and the per-package-only constraint on the four `parse_list_map`-based allowlists. Updated Decision Model step 5 to list all six allowlists and note the effective-set computation.

### Finding 375: No convention for line numbers in finding entries
- **Root Cause**: Finding entries in `FIXED_FINDINGS.md`, `WONT_FIX_FINDINGS.md`, and `OPEN_FINDINGS.md` inconsistently included specific line numbers (e.g. `src/lib.rs:210`, `src/scanning.rs:295-304`). These drift silently after future edits. No guidance existed on whether line numbers were approximate navigational aids or authoritative references, leaving reviewers uncertain whether a stale pointer indicated a documentation gap or a code move.
- **Fix**: Added a convention to AGENTS.md step 9: omit line numbers from finding entries — file names only (e.g. `src/lib.rs`, not `src/lib.rs:210`). Line numbers are not a contract; `grep` is the correct tool for locating current positions.

### Finding 32: NUL-byte path truncation bypass in strace path unescaping
- **Root Cause**: `unescape_strace_string` converts `\x00` to byte `0`, then `String::from_utf8_lossy` preserved the NUL byte or replaced invalid bytes with `U+FFFD`. In Linux syscalls, path arguments are NUL-terminated C strings. An attacker opening `/etc/passwd\x00harmless.txt` caused the kernel to open `/etc/passwd`, but GyrSeek's `is_sensitive_file_read` checks (`ends_with` and `exact_match`) evaluated against `"/etc/passwd\0harmless.txt"`, failing to match and completely bypassing sensitive file detection.
- **Fix**: In `extract_sensitive_file_reads` and `extract_process_exec_signatures`, truncated the unescaped byte slice at the first NUL byte (`bytes.split(|&b| b == 0).next().unwrap_or(&[])`) prior to converting to `String`. Added unit tests verifying NUL-byte truncation in both path extraction and process argv extraction.
