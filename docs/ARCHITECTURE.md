...
- Behavior capture:
  ...
  - `NonoRunner` backend executes probes using [`nono`](https://github.com/nolabs-ai/nono) (Linux Landlock LSM / macOS Seatbelt), enforcing kernel-level capability boundaries (`--allow <probe_dir>`, `--allow-net`) and parsing tamper-evident audit logs into `TraceSignals` with post-install artifact classification executed directly in Rust without `strace` or `ptrace` context-switch overhead.
  - `build_runner_from_env` selects backend mode (`docker` default, `microvm`, `nono`, or `host` fallback).
- Anomaly decision:
