...
The CI pipeline (`ci.yml` and `post_review.yml`) employs a strict trusted/untrusted architectural boundary to prevent supply chain and prompt injection attacks during automated code review:
- **Untrusted Phase (`ci.yml`)**: Triggered by `pull_request`, this phase runs strictly with read-only (`contents: read`) permissions. It checks out the untrusted PR code, runs the test suite, and executes the AI code review tools (`opencode`). To prevent prompt injection vulnerabilities, AI tooling is restricted to reading workspace files dynamically via its tools, rather than relying on brittle XML-tag prompt injections. All outputs (the review ledger and consolidated report) are uploaded as neutral artifacts.
- **Trusted Phase (`post_review.yml`)**: Triggered by `workflow_run`, this phase runs in a separate, isolated runner with `pull-requests: write` permissions. It securely downloads the artifacts generated by the untrusted phase and posts them to the PR.
