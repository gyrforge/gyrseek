...
- Network isolation failure: container cannot reach package registries due to `--network none`.

## Docker Sandbox Security
- Seccomp profile (stored inline in Rust in `src/sandbox.rs`, materialized to temp at runtime) denies high-risk syscalls while preserving strace/ptrace compatibility and allowing package-manager network access to registries.
