...
Gyrseek maintains a strict security boundary between untrusted PR code and privileged repository operations:
- **`ci.yml` (Untrusted Execution):** Triggered `on: pull_request`. This workflow runs with workflow-level `contents: read` permissions (with a scoped job-level `checks: write` exception granted strictly to the `cargo-audit` job).
- Automated code reviews are handled externally via CodeRabbit (`.coderabbit.yaml`).
