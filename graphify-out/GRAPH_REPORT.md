...
## Corpus Check
- 24 files · ~44,161 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 588 nodes · 1263 edges · 29 communities (22 shown, 7 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 34 edges (avg confidence: 0.82)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3b425b5a`
...
Cohesion: 0.07
Nodes (37): Shai-Hulud Attack Class, Watched-Process Execution Detection, Shai-Hulud Attack, artifact_findings_empty_for_clean_install(), case_1_new_bun_is_flagged_against_clean_baseline(), case_2_existing_bun_plus_additional_invocation_is_flagged(), case_2b_changed_bun_arguments_are_flagged(), classify_inventory_benign_pth() (+29 more)

### Community 4 - "Git-Clone & Process-Exec Signatures"
Cohesion: 0.10
Nodes (33): Behavioral Diffing Across Versions, Finding 3: Argv Regex Truncation at ], Direct Git Clone Runtime Interception, F, HashSet, PyPiReleaseFile, artifact_allowlist_matches_exact_finding_and_prefix(), detects_anomalous_new_connection() (+25 more)
...
Cohesion: 0.10
Nodes (19): Adding a New Supported Command, Build and Test, Developer Guide, just Task Runner, Local Setup, Policy Config Surface, Practical Review Checklist, Required Change Hygiene (+11 more)

### Community 11 - "Domain Allowlist & DNS Enrichment"
Cohesion: 0.13
Nodes (16): Finding 10: Self-Referencing Baseline Override (Open), baseline_count_limits_fetched_baselines_without_overrides(), baseline_count_zero_returns_no_effective_baselines(), burst_policy_emits_warning_when_triggered(), burst_policy_warning(), burst_triggered(), duplicate_override_versions_are_deduped_and_truncated(), minimum_release_age_policy_warning() (+8 more)
...
Cohesion: 0.05
Nodes (40): Architecture, Core Components, Current Limitations, Decision Model, Docker Sandbox Security, Empty-Trace Hard Error, Fail-Closed Policy, Goal (+32 more)
...
Cohesion: 0.50
Nodes (4): filter_allowlisted_git_clone_signatures(), git_clone_allowlist_does_not_match_different_url(), git_clone_allowlist_matches_non_recursive_clone_of_allowed_url(), git_clone_allowlist_matches_recursive_clone_of_allowed_url()
...
## Knowledge Gaps
- **138 isolated node(s):** `$schema`, `plugin`, `BaselineOverrideConfig`, `Drop`, `ForwardMode` (+133 more)
...
- **Why does `run()` connect `Routing, Config & Scan Orchestration` to `Fail-Closed Policy & Findings`, `Package Parsing & Version Pinning`, `Host Command Forwarding`, `Sandbox Backends & Docker/strace`?**
  _High betweenness centrality (0.224) - this node is a cross-community bridge._
- **Why does `trace_sandbox_install_matrix()` connect `Git-Clone & Process-Exec Signatures` to `Sandbox Backends & Docker/strace`, `Connection IP Extraction & Filtering`, `Bun Execution Detection & Exemptions`, `Registry History & Baseline Selection`, `Fail-Closed Forwarding Tests`?**
  _High betweenness centrality (0.109) - this node is a cross-community bridge._
- **Why does `SandboxRunner` connect `Sandbox Backends & Docker/strace` to `Routing, Config & Scan Orchestration`, `Git-Clone & Process-Exec Signatures`?**
