...
- CI:
  - `.github/workflows/ci.yml` — Runs untrusted PR code with workflow-level `contents: read` permissions (with a scoped job-level `checks: write` exception granted strictly to the `cargo-audit` job). Includes `rust-checks` (lint, test), security audits (`cargo-audit`), and containerized test matrices (`linux-docker-uv-test`, etc.). Automated code reviews are handled via CodeRabbit (`.coderabbit.yaml`).
- Build and scripts:
  ...
  - just test-nono-{npm,pnpm,pip,uv,poetry} / just test-nono — end-to-end tests per manager using the release binary (experimental nono sandbox)
- Test strategy:
...
- Current behavior highlights:
  ...
  - Non-registry npm/pnpm CLI specifications (`file:`, `git+`, `https://`, `link:`, `./`, `../`, etc.): when all positional CLI targets are non-registry specs, gyrseek forwards the command directly without triggering the `package.json` fallback (FIXED_FINDINGS.md #11) or failing closed with exit code 1 when no `package.json` exists (FIXED_FINDINGS.md #12).
  - Container memory and `/work` tmpfs limit: configurable via `GYRSEEK_MEM_LIMIT` (default `2g`), preventing OOM-kills during heavy native dependency compilation in npm/pnpm/python builds (was FIXED_FINDINGS.md #21).
  ...
  - Behavioral anomaly detection traces `open`, `openat`, and related system calls via `strace` to monitor for attempts to read highly sensitive credential or configuration files (e.g., `~/.aws/credentials`, `~/.npmrc`, `.env`). Syscall return codes are verified (`ret_val >= 0`) so failed accesses never populate baseline traces (closing the baseline poisoning gap, was FIXED_FINDINGS.md #60). If a package accesses sensitive files that older versions did not touch, the install is immediately flagged.
  ...
  - process_exec_allowlist allows specific watched-process signatures (`bun|run|build`) or bare executables (`bun`) even when newly introduced (case-insensitive)
    - Test-only env vars `GYRSEEK_TEST_FORCE_BASELINE_AGES_HOURS`, `GYRSEEK_TEST_FORCE_RELEASES_LAST_24H`, `GYRSEEK_TEST_FORCE_CURRENT_RELEASE_AGE_DAYS`, and `GYRSEEK_TEST_ECHO_MIN_BASELINE_AGE_HOURS` in `fetch_history_with_baselines` at `src/scanning.rs` are gated with `#[cfg(any(debug_assertions, test))]` (compile-time, not runtime `cfg!()`) so they are literally absent from release binaries — no dead branch to patch, no optimizer-dependence. In debug builds and test builds (`cargo build`/`cargo run`/`cargo test`), these env vars remain active and a runtime warning is emitted when any are set.
  ...
  - New IPs remain fail-closed anomalies (both IPv4 and IPv6 connection endpoints are captured from trace output); warning output shows each IP enriched with its FCrDNS domain inline (e.g. `203.0.113.42 -> suspicious-c2.example`), rather than as a separate informational footnote
   - extract_connection_ips normalizes endpoints via normalize_ip_string (canonical IPv6 plus IPv4-mapped IPv6 `::ffff:1.2.3.4` collapsed to bare IPv4) and drops sandbox-local addresses via is_sandbox_local_ip (loopback, link-local `fe80::/10` + `169.254/16`, and private/RFC1918 incl. the Docker bridge `172.17/16` and Docker Desktop gateway `192.168.65/24`) at capture time, before the baseline diff and on both current and baseline traces — this removed a whole class of harness-nondeterminism false positives where the container's own gateway/DNS showed up as a "new" endpoint. The cloud metadata IP `169.254.169.254` is exempt (kept) as a real SSRF/credential-theft signal. The ip_allowlist matcher also compares on the collapsed form, so `172.17.0.2` and `::ffff:172.17.0.2` match interchangeably.
     - DNS wire-format parser (unescape_strace_string, decode_dns_name with recursive 0xc0 compression pointer support; circular pointer detection via pointer_count limit of 5 hops; parse_dns_response, extract_dns_map — UDP and TCP DNS responses) in scanning.rs. Requires strace `-xx` flag for deterministic hex-escape parsing. `extract_dns_map` handles mixed IPv4 (`sin_port`) and IPv6 (`sin6_port`) sockets interchangeably and skips short TCP reads (< 3 bytes) without crashing, but lacks full TCP reassembly (fragmented DNS responses are dropped). TCP `recvmsg` capture uses anchor `msg_iov=\[\{(?:iov_base=)?` to handle both bare-buffer and `iov_base=`-prefixed strace `-v` output formats (was FIXED_FINDINGS.md #272).
   - `check_override_ages` and `filter_override_version` enforce a 24-hour hard minimum age gate on baseline override versions in the production path (called from `scan_packages_versions` at `src/scanning.rs`). Overrides younger than `HARD_MINIMUM_AGE_HOURS` are rejected with a warning before reaching `select_effective_baselines`. `select_effective_baselines` returns a `(Vec<String>, bool)` tuple where the boolean flag denotes if an override self-referenced the `current` version, coupling the enforcement tightly to the caller's warning.
  - `HARD_MINIMUM_AGE_HOURS` and `DEFAULT_MIN_BASELINE_AGE_HOURS` are `i64` (not `u64`) to align with `chrono::Duration::hours()` return type, eliminating `as i64` casts at multiple comparison sites.
  ...
  - YAML policy config is supported (`gyrseek.yaml` by default, overridable via `--config`/`-c` or `GYRSEEK_CONFIG`) using:
    - `ip_allowlist` — supports both global entries (`- "1.2.3.4"`) and per-package entries (`- pkg: ["5.6.7.8"]`) under the same key; effective allowlist for a scan is global ∪ per-package (IPs are canonicalized so equivalent IPv6 representations match)
    - `domain_allowlist` — same mixed global/per-package syntax as `ip_allowlist`; `"*"` as a value (not a key) emits a warning and is dropped — it matches nothing in `domain_is_allowlisted`; bare labels without a dot (e.g. `"com"`) are rejected at config-load time with a warning — they would match all `.com` domains via the `ends_with(".com")` suffix check
    - `git_clone_allowlist` (allowlist for install-time git clone targets; per-package only, no global `"*"` key — using `"*"` as a key emits a warning and is dropped)
    - `artifact_allowlist` (exact `type|path|details` or prefix `type|path` to unblock known artifact findings; per-package only, no global `"*"` key)
    - `sensitive_file_access_allowlist` (allowlist specific paths that packages are allowed to read, e.g., for AWS SDKs to read credentials; per-package only, no global `"*"` key)
    - optional package `baseline_overrides` (`baseline-1`/`baseline-2`; overrides younger than `HARD_MINIMUM_AGE_HOURS`=24h are rejected with a warning and stripped from the effective baseline set — `check_override_ages` now returns a `(warnings, filtered)` tuple to propagate the cleaned set)
    - `baseline_count` (default 2)
    - per-package `min_baseline_age_hours` (default effective age gate 72 hours; any value below `HARD_MINIMUM_AGE_HOURS`=24 is clamped with a warning — the floor is enforced at config-parse time in `src/lib.rs`; the already-clamped value is passed through to `fetch_history_with_baselines`)
    - `new_package_exemptions` (map format `pkg: "version"` of package names to a specifically vetted version; a custom `deserialize_new_package_exemptions` deserializer accepts the map format with backward compat via an untagged enum, emitting a custom `InvalidMap` error message if non-string values are provided instead of an opaque enum failure; the deprecated list format `[pkg]` is rejected with a hard config-parse error and must be migrated to map format; empty version values are rejected with a warning; the exemption only matches the resolved `v_curr` — the former `tgt_version` comparison was removed to prevent a bypass where `"latest"` as an exemption value matched unpinned installs; bypasses anomaly block when <2 eligible baselines exist, but only applies to the exact pinned version)
    - `internal_package_exemptions` (skip a package entirely — no registry fetch, no sandbox install, no diff; for first-party/private-index packages e.g. Nexus that public-registry lookups can't resolve; forwarded unscanned at the requested version)
    - optional `minimum_release_age_package` (disabled by default; 0 silently falls back to disabled)
    - optional `release_burst_threshold` (disabled by default; 0 silently falls back to disabled)
    - optional `release_burst_window_hours` (default 24h; 0 silently falls back to 24h)
    - `process_exec_allowlist` (allow specific watched-process signatures or bare executables; per-package only, no global `"*"` key)
  - All allowlist config entries validate package keys at parse time: blank/whitespace-only keys and the reserved `"*"` key in per-package position emit a `⚠️ [gyrseek]` warning and are dropped (ip_allowlist, domain_allowlist, and all parse_list_map-based allowlists); whitespace-variant keys that trim to the same value emit a collision warning and are merged (not overwritten)
  - `validate_allowlist_pkg_key(pkg_raw, allowlist_name, existing, has_global_variant)` helper in `src/lib.rs`: trims, rejects empty, rejects `"*"` with guidance text (ip/domain: "use a bare global entry instead"; parse_list_map callers: "this allowlist does not support a global wildcard"), warns on whitespace-variant collision, returns `Some(trimmed_key)` or `None`; used by ip_allowlist PerPackage (`true`), domain_allowlist PerPackage (`true`), and `parse_list_map` (`false`)
  - `parse_list_map(map, lowercase, name)` (used for git_clone_allowlist, artifact_allowlist, sensitive_file_access_allowlist, process_exec_allowlist) uses `entry().or_default().extend()` merge semantics, consistent with the ip/domain PerPackage branches — duplicate or whitespace-variant keys merge their value sets with a warning; each call site passes its specific allowlist name so diagnostics name the allowlist correctly (e.g. `git_clone_allowlist key "pkg" trimmed...`); emits a warning when a package key's value set is empty after filtering (operator has no protection for that key)
  - `sensitive_file_access_allowlist` values are validated at config-load time: `"*"`, `"/"`, `"*/"`, and `"/*"` are dropped with a per-package warning immediately after `parse_list_map` returns; a second `retain` pass then removes any package keys whose entire value set was filtered out (emits a per-package warning); the filter receives only pre-validated entries
  - `option_zero_to_none(val: Option<usize>, warn_msg: &str) -> Option<usize>` helper in `src/lib.rs`: maps `Some(0) → None` with warning, passes through `Some(v)` and `None` unchanged; used for `release_burst_threshold`, `minimum_release_age_package`, and `release_burst_window_hours` (bridged to `usize` via `.unwrap_or(24)`)
  - `find_new_items(current: &HashSet<String>, baseline: &HashSet<String>) -> Vec<String>` helper in `src/scanning.rs`: set difference + sort; `find_new_sensitive_reads`, `find_new_git_clone_signatures`, and `find_new_process_exec_signatures` all delegate to it
  - `filter_allowlisted_sensitive_reads` signature is `(package_name, reads, allowlist_map)`, consistent with all other filter functions
  - ip_allowlist entries are normalized via `normalize_ip_string` at config-load time (IPv4-mapped `::ffff:x.x.x.x` collapsed to bare IPv4); the allowlist map always holds canonical forms so no re-parsing is needed at filter call time
  ...
  - GYRSEEK_SANDBOX supports `nono` (Landlock LSM on Linux 5.13+, Seatbelt on macOS; experimental backend — use at your own risk) via NonoRunner:
    - Scoped network egress: baseline probes harvest observed domains; candidate probes are strictly confined to official registries, configured registry domains, and baseline domains via `--allow-domain` (no blanket egress)
    - OS filesystem isolation: macOS ephemeral Seatbelt profile denies `/tmp`, `/private/tmp`, and user cache dirs (`/var/folders/.../C/`); Linux Landlock excludes `system_write_linux` (denying host `/tmp` writes) with cgroups v2 resource limits (`2g` memory, `256` max processes)
    - Detection parity: `CanaryTrap` FIFO decoys for credential theft detection (`.env`, `~/.aws/credentials`, `~/.ssh/id_rsa`, `~/.npmrc`); `PathShims` capturing untruncated argv for `bun`, `deno`, `git`, `curl`, `wget`; supervisor diagnostic denial parsing; and host-side artifact scan (`scan_target_artifacts`) feeding `classify_inventory_lines`
...
8. If test structure or coverage changes significantly, update docs/TESTS.md.
 9. If a new finding is identified, add it to `docs/OPEN_FINDINGS.md` and its detailed rationale to `docs/OPEN_FINDINGS_DETAILED.md`. When a finding is fixed, move it to `docs/FIXED_FINDINGS.md` and `docs/FIXED_FINDINGS_DETAILED.md`. If it is excluded from fixing, move it to the `WONT_FIX` equivalents. Summary tables live only in the main files (`FIXED_FINDINGS.md`, `WONT_FIX_FINDINGS.md`, `OPEN_FINDINGS.md`); the `_DETAILED.md` files contain only prose entries. All finding IDs use a single flat numeric namespace (no category prefixes). When adding new findings, choose the next available number across all three categories to avoid collisions. **Omit line numbers from finding entries** — they drift as code evolves and are not a contract; use the file name only (e.g. `src/lib.rs`, not `src/lib.rs:210`).
