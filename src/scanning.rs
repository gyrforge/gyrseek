...
"#,
    ...
    fn classify_inventory_binary_elf() {
        let raw: HashSet<String> = ["/usr/bin/somebin|12345|ELF 64-bit LSB executable x86-64|..."]
            .iter()
            .map(|s| s.to_string())
            .collect();
    ...
    fn classify_inventory_unexpected_runtime() {
        let raw: HashSet<String> = [
            "/work/bun-x64|5678|ELF 64-bit LSB executable|...",
            "/work/node|1234|ELF 64-bit LSB executable|...",
        ]
        .iter()
        .map(|s| s.to_string())
        .collect();
    ...
    fn classify_inventory_suspicious_pth() {
        let raw: HashSet<String> =
            ["/work/site-packages/evil.pth|100|ASCII text|import socket subprocess"]
                .iter()
                .map(|s| s.to_string())
                .collect();
        let out = classify_inventory_lines(raw);
    ...
    fn classify_inventory_benign_pth() {
        let raw: HashSet<String> =
            ["/work/site-packages/happy.pth|50|ASCII text|# just a path\n../../lib"]
                .iter()
                .map(|s| s.to_string())
                .collect();
        let out = classify_inventory_lines(raw);
    ...
    fn classify_inventory_large_file() {
        let raw: HashSet<String> = ["/work/data.bin|20971520|data|..."]
            .iter()
            .map(|s| s.to_string())
            .collect();
        let out = classify_inventory_lines(raw);
    ...
    fn classify_inventory_skips_malformed_lines() {
        let raw: HashSet<String> = ["not-enough-fields", "/work/ok|10||"]
            .iter()
            .map(|s| s.to_string())
            .collect();
        let out = classify_inventory_lines(raw);
    ...
    fn classify_inventory_mixed_findings() {
        let raw: HashSet<String> = [
            "/work/bun|1000|ELF 64-bit LSB executable|...",
            "/work/hack.pth|300|ASCII text|import urllib.request",
            "/work/big.bin|10485761|data|...",
            "/work/normal.py|200|ASCII text|print('hello')",
        ]
        .iter()
        .map(|s| s.to_string())
        .collect();
        ...
        let runner = MockRunner {
            traces: HashMap::from([
                (
                    ...
                    format!(
                        "sin_addr=inet_addr(\"1.2.3.4\")\n{}/work/site-packages/evil.pth|150|ASCII text|import socket",
    ...
    async fn allows_when_artifact_findings_match_baseline() {
        ...
        let pth = format!(
            "some strace{}/work/pkg.pth|80|ASCII text|import helper",
            crate::scanning::ARTIFACT_DELIMITER
        );
    ...
    async fn new_package_exemption_skips_artifact_check() {
        ...
        let runner = MockRunner {
            traces: HashMap::from([(
                ...
                format!(
                    "some strace{}/work/evil.pth|200|ASCII text|import urllib",
                    crate::scanning::ARTIFACT_DELIMITER
                ),
            )]),
        };
